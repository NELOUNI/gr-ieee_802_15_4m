...
if __name__ == '__main__':      
        ...
        parser.add_option(""  ,"--uhd-dir"         , default=os.getenv("GNURADIO"), help="installation directory of uhd, [default=%default]")
	...
	c.setopt(pycurl.CAINFO, "keys/antd.nist.gov.crt")
        ...
        subprocess.call(options.uhd_dir+'/lib64/uhd/examples/tx_samples_from_file --args ' + options.usrp_addr + ' --file ' + options.video + ' --type short --rate ' + str(rate) + ' --gain 20 --freq ' + str(freq) + ' --repeat ', shell=True)
        ctx.pop()
